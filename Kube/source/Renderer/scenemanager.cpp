...
void SceneManager::updateLightUniform(Shader *shader) {
    ...
    //!< \todo Incorporate this into shader class after generic shader uniform setting is implemented
    for (size_t i = 0; i < m_lights.size(); i++) {
        std::stringstream ss;
        ss << "lights[" << i << "].position";
        shader->setUniform4f(ss.str().c_str(), m_lights[i]->position.x, m_lights[i]->position.y, m_lights[i]->position.z, m_lights[i]->position.w);

        ss.str("");
        ss << "lights[" << i << "].color";
        shader->setUniform3f(ss.str().c_str(), m_lights[i]->color.r, m_lights[i]->color.g, m_lights[i]->color.g);
