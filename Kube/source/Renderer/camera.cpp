...
void Camera::setHorizontalAngle(float radians)
{
	m_horizontalAngle = radians;
}

void Camera::setVerticalAngle(float radians)
{
	m_verticalAngle = radians;
}
...
void Camera::update() {
    ...
    m_viewMatrix = glm::lookAt(
            m_cameraPosition,
            m_cameraPosition + m_vectorForward,
            m_vectorUp
    );
}
...
void Camera::setOrthogonalProjection(float windowWidth, float windowHeight) {
	m_projectionMatrix = glm::ortho(0.0f, windowWidth, windowHeight, 0.0f, 0.1f, 1500.0f);
}
