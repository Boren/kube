...
bool Engine::initialize(std::string windowName) {
  ...
  std::stringstream ss;
  ss << "Renderer: " << glGetString(GL_VENDOR) << " - "
     << glGetString(GL_RENDERER);
  Log::message(Log::DEBUG, "Engine", ss.str());

  ss.str("");
  ss << "OpenGL " << glGetString(GL_VERSION) << ", GLSL "
...
SceneManager *Engine::getSceneManager() { return pSceneManager; }

InputManager *Engine::getInputManager() { return pInputManager; }

Renderer *Engine::getRenderer() { return pRenderer; }

void Engine::setCamera(Camera *camera) { pCamera = camera; }

Window *Engine::getWindow() { return pWindow; }
