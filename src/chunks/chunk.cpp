...
void Chunk::load() {
  std::stringstream ss;
  ss << "Loading chunk (" << m_chunkCoord.x << "," << m_chunkCoord.y << ","
     << m_chunkCoord.z << ")";
  Log::message(Log::DEBUG, "Chunk", ss.str());

  m_blocks = new Block[ChunkManager::CHUNK_SIZE * ChunkManager::CHUNK_SIZE *
...
bool Chunk::isMeshed() { return m_isMeshed; }
...
Block *Chunk::getBlock(glm::ivec3 coord) {
  ...
  if (coord.x >= ChunkManager::CHUNK_SIZE) {
    if (m_front == nullptr)
      return nullptr;
    return m_front->getBlock(coord -
                             glm::ivec3(ChunkManager::CHUNK_SIZE, 0, 0));
  }

  if (coord.y < 0) {
    if (m_bottom == nullptr)
      return nullptr;
    return m_bottom->getBlock(coord +
                              glm::ivec3(0, ChunkManager::CHUNK_SIZE, 0));
  }
  ...
  if (coord.z < 0) {
    if (m_right == nullptr)
      return nullptr;
    return m_right->getBlock(coord +
                             glm::ivec3(0, 0, ChunkManager::CHUNK_SIZE));
  }
  ...
  return &m_blocks[coord.x * ChunkManager::CHUNK_SIZE *
                       ChunkManager::CHUNK_SIZE +
                   coord.y * ChunkManager::CHUNK_SIZE + coord.z];
}

void Chunk::buildMesh() {
  std::stringstream ss;
  ss << "Meshing chunk (" << m_chunkCoord.x << "," << m_chunkCoord.y << ","
     << m_chunkCoord.z << ")";
  Log::message(Log::DEBUG, "Chunk", ss.str());
    ...
    for (int y = 0; y < ChunkManager::CHUNK_SIZE; y++) {
      for (int z = 0; z < ChunkManager::CHUNK_SIZE; z++) {
        ...
        if (currentBlock->isActive()) {
          createBlockMesh(
              currentBlock,
              glm::ivec3(ChunkManager::CHUNK_SIZE * m_chunkCoord.x + x,
  ...
  if (m_numberOfVertices != 0) {
    ...
    glBufferData(GL_ARRAY_BUFFER,
                 m_ambientOcclusionValues.size() * sizeof(float),
...
void Chunk::createBlockMesh(Block *block, glm::ivec3 blockCoord,
  ...
  glm::vec3 p1(blockCoord.x - blockSizeHalf, blockCoord.y - blockSizeHalf,
               blockCoord.z + blockSizeHalf);
  glm::vec3 p2(blockCoord.x + blockSizeHalf, blockCoord.y - blockSizeHalf,
               blockCoord.z + blockSizeHalf);
  glm::vec3 p3(blockCoord.x + blockSizeHalf, blockCoord.y + blockSizeHalf,
               blockCoord.z + blockSizeHalf);
  glm::vec3 p4(blockCoord.x - blockSizeHalf, blockCoord.y + blockSizeHalf,
               blockCoord.z + blockSizeHalf);
  glm::vec3 p5(blockCoord.x + blockSizeHalf, blockCoord.y - blockSizeHalf,
               blockCoord.z - blockSizeHalf);
  glm::vec3 p6(blockCoord.x - blockSizeHalf, blockCoord.y - blockSizeHalf,
               blockCoord.z - blockSizeHalf);
  glm::vec3 p7(blockCoord.x - blockSizeHalf, blockCoord.y + blockSizeHalf,
               blockCoord.z - blockSizeHalf);
  glm::vec3 p8(blockCoord.x + blockSizeHalf, blockCoord.y + blockSizeHalf,
               blockCoord.z - blockSizeHalf);

  // We don't want to render sides which are occluded by other blocks so we only
  // draw that side if the neighbor block is not active.
  if (!blockActive(blockCoordLocal + glm::ivec3(1, 0, 0)))
    createFaceMesh(glm::vec3(1, 0, 0), p2, p5, p8, p3, color, blockCoordLocal);
  if (!blockActive(blockCoordLocal + glm::ivec3(-1, 0, 0)))
    createFaceMesh(glm::vec3(-1, 0, 0), p6, p1, p4, p7, color, blockCoordLocal);
  if (!blockActive(blockCoordLocal + glm::ivec3(0, 1, 0)))
    createFaceMesh(glm::vec3(0, 1, 0), p4, p3, p8, p7, color, blockCoordLocal);
  if (!blockActive(blockCoordLocal + glm::ivec3(0, -1, 0)))
    createFaceMesh(glm::vec3(0, -1, 0), p6, p5, p2, p1, color, blockCoordLocal);
  if (!blockActive(blockCoordLocal + glm::ivec3(0, 0, 1)))
    createFaceMesh(glm::vec3(0, 0, 1), p1, p2, p3, p4, color, blockCoordLocal);
  if (!blockActive(blockCoordLocal + glm::ivec3(0, 0, -1)))
    createFaceMesh(glm::vec3(0, 0, -1), p5, p6, p7, p8, color, blockCoordLocal);
}

void Chunk::createFaceMesh(glm::vec3 normal, glm::vec3 p1, glm::vec3 p2,
  ...
  // Ambient Occlusion
  for (int i = 0; i < 6; i++) {
    /*m_ambientOcclusionValues.push_back(getAmbientOcclusionAtVertex(blockCoordLocal,
    normal, glm::vec3(-1, 0, 0), glm::vec3(0, -1, 0)));
    m_ambientOcclusionValues.push_back(getAmbientOcclusionAtVertex(blockCoordLocal,
    normal, glm::vec3(1, 0, 0), glm::vec3(0, -1, 0)));
    m_ambientOcclusionValues.push_back(getAmbientOcclusionAtVertex(blockCoordLocal,
    normal, glm::vec3(1, 0, 0), glm::vec3(0, -1, 0)));
...
int Chunk::getAmbientOcclusionAtVertex(glm::vec3 position,
                                       glm::vec3 normalDirection,
                                       glm::vec3 direction1,
                                       glm::vec3 direction2) {
  Block *side1 =
      getBlock(glm::ivec3(position.x + normalDirection.x + direction1.x,
                          position.y + normalDirection.y + direction1.y,
                          position.z + normalDirection.z + direction1.z));

  Block *side2 =
      getBlock(glm::ivec3(position.x + normalDirection.x + direction2.x,
                          position.y + normalDirection.y + direction2.y,
                          position.z + normalDirection.z + direction2.z));

  Block *corner = getBlock(
      glm::ivec3(position.x + normalDirection.x + direction1.x + direction2.x,
  ...
  if (side1 != nullptr)
    side1active = side1->isActive();
  if (side2 != nullptr)
    side2active = side2->isActive();
  if (corner != nullptr)
    corneractive = corner->isActive();
