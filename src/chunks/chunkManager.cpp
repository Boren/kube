...
void ChunkManager::update(float deltaTime, Camera *camera) {
  ...
  for (int x = currentChunkCoord.x - m_loadDistance;
       x <= currentChunkCoord.x + m_loadDistance; x++) {
    for (int y = currentChunkCoord.y - m_loadDistance;
         y <= currentChunkCoord.y + m_loadDistance; y++) {
      for (int z = currentChunkCoord.z - m_loadDistance;
           z <= currentChunkCoord.z + m_loadDistance; z++) {
  ...
  for (int x = currentChunkCoord.x - m_viewDistance;
       x <= currentChunkCoord.x + m_viewDistance; x++) {
    for (int y = currentChunkCoord.y - m_viewDistance;
         y <= currentChunkCoord.y + m_viewDistance; y++) {
      for (int z = currentChunkCoord.z - m_viewDistance;
           z <= currentChunkCoord.z + m_viewDistance; z++) {
        glm::ivec3 chunkCoord(glm::ivec3(x, y, z));

        if (m_chunks.find(chunkCoord) != m_chunks.end()) {
          if (m_chunks[chunkCoord]->isLoaded() &&
              !m_chunks[chunkCoord]->isMeshed())
...
void ChunkManager::setChunkLoadCallback(
    std::function<void(Chunk *)> callbackFunction) {
